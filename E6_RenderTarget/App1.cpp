...
void App1::init(HINSTANCE hinstance, HWND hwnd, int screenWidth, int screenHeight, Input *in, bool VSYNC, bool FULL_SCREEN)
{
	...
	orthoMesh_2 = new OrthoMesh(renderer->getDevice(), renderer->getDeviceContext(), screenWidth / 4, screenHeight / 4, screenWidth / 2.7, screenHeight / 2.7);
	noTextureShader = new NoTextureShader(renderer->getDevice(), hwnd);
	...
	secondCamera->setPosition(0.0f, 10.0f, 0.0f);
	secondCamera->setRotation(90,0,0);
	secondCamera->update();

	thirdCamera = new Camera();
	thirdCamera->setPosition(50.0f, 10.0f, 0.0f);
	thirdCamera->setRotation(90, 0, 0);
	thirdCamera->update();
...
void App1::firstPass()
{
	...
	noTextureShader->setShaderParameters(renderer->getDeviceContext(), worldMatrix, viewMatrix, projectionMatrix, textureMgr->getTexture(L"default"), light);
	noTextureShader->render(renderer->getDeviceContext(), cubeMesh->getIndexCount());
